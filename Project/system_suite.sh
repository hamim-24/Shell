...
# Author: Generated by GPT-5.1 Codex
...
file_editor_nvim() {
  ...
  printf "3) Browse with fzf\n"
  read -r -p "Select option [1/2/3]: " editor_choice
  ...
  case "${editor_choice}" in
    ...
    3)
      ...
      use_fzf=true
      create_new=false
      printf "\nBrowsing files with fzf (type to filter, Enter to select, Esc to cancel)\n"
      local selected
      set +e
      if command -v fd >/dev/null 2>&1; then
        selected=$(fd --type f --hidden=false --exclude node_modules --exclude Library --max-depth 5 "${HOME}" 2>/dev/null | fzf --height 40% --border 2>/dev/null)
      else
        selected=$(find "${HOME}" -type f -maxdepth 5 -not -path '*/\.*' -not -path '*/node_modules/*' -not -path '*/Library/*' -not -path '*/\.cache/*' 2>/dev/null | fzf --height 40% --border 2>/dev/null)
      fi
      local fzf_exit=$?
      set -e
      ...
      if [[ ${fzf_exit} -ne 0 ]]; then
        ...
        if [[ ${fzf_exit} -eq 130 ]]; then
          notify_info "Search cancelled"
        elif [[ ${fzf_exit} -eq 1 ]]; then
          notify_info "No file selected"
        elif [[ ${fzf_exit} -eq 141 ]]; then
          notify_warn "Too many files. Try searching in a specific directory."
        else
          notify_warn "fzf error (exit code: ${fzf_exit})"
        fi
        pause
        return
      fi
      ...
      if [[ -n ${selected} ]] && [[ -f ${selected} ]]; then
        ...
        clear_screen
        print_menu_header
        notify_warn "Invalid file selected"
        pause
        return
      fi
      ;;
    *)
      notify_warn "Invalid option"
      pause
      return
      ;;
  esac
